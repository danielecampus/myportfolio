...
# analysis
ret_pure <- multiasset_sheet_returns %>% select(-Dates)
...
write_parquet(ptf_output$Ptf_Summary, paste0(output_path, "multiasset_summary.parquet"))
...
# Var - ES
multiasset_VaR <- VaR(multiasset_sheet_returns, p = 0.95, portfolio_method = "component", weights = multiasset_quotes)
multiasset_ES <- ES(multiasset_sheet_returns, p = 0.95, portfolio_method = "component", weights = multiasset_quotes)
